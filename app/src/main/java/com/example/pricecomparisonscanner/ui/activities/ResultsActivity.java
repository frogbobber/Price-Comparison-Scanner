...
public class ResultsActivity extends AppCompatActivity {
    ...
    private ArrayList<AllProductInformation> JSONtoAllProductInformation(ArrayList<String> data) throws JSONException {
        ...
        ArrayList<ProductInformation> bestbuyArr = new ArrayList<ProductInformation>();
        ...
        for(int i = 0; i < data.size(); i++){
            ...
            if(obj.has("targetProducts")){
                JSONArray targetProducts = obj.getJSONArray("targetProducts");
                for (int j = 0; j < targetProducts.length(); j++) {
                    targetArr.add(new ProductInformation(targetProducts.getJSONObject(j).getString("url"),
                            targetProducts.getJSONObject(j).getString("name"), targetProducts.getJSONObject(j).getString("price")));
                }
            }
            if(obj.has("walmartProducts")){
                JSONArray walmartProducts = obj.getJSONArray("walmartProducts");
                for (int j = 0; j < walmartProducts.length(); j++) {
                    walmartArr.add(new ProductInformation(walmartProducts.getJSONObject(j).getString("url"),
                            walmartProducts.getJSONObject(j).getString("name"), walmartProducts.getJSONObject(j).getString("price")));
                }
            }
            if(obj.has("upciteProducts")){
                JSONArray upciteProducts = obj.getJSONArray("upciteProducts");
                for (int j = 0; j < upciteProducts.length(); j++) {
                    upciteArr.add(new ProductInformation(upciteProducts.getJSONObject(j).getString("url"),
                            upciteProducts.getJSONObject(j).getString("name"), upciteProducts.getJSONObject(j).getString("price")));
                }
            }

            if(obj.has("bestbuyProducts")){
                JSONArray bestbuyProducts = obj.getJSONArray("bestbuyProducts");
                for (int j = 0; j < bestbuyProducts.length(); j++) {
                    bestbuyArr.add(new ProductInformation(bestbuyProducts.getJSONObject(j).getString("url"),
                            bestbuyProducts.getJSONObject(j).getString("name"), bestbuyProducts.getJSONObject(j).getString("price")));
                }
            }
